...
import { BASE_URLS, DEFAULT_RETRY_ATTEMPTS, DEFAULT_TIMEOUT, Environments } from "./constants";
...
export class KonnectSDK {
  ...
  constructor(config: KonnectConfig) {
    ...
    this.baseUrl = BASE_URLS[config.environment || Environments.SANDBOX];
  ...
  private async request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
    ...
    // Safely merge incoming headers based on their type
    if (options.headers) {
   ...
   * ```
   */
  async initPayment(request: InitPaymentRequest): Promise<ApiResponse<InitPaymentResponse>> {
  ...
  async isPaymentCompleted(paymentId: string): Promise<ApiResponse<boolean>> {
    return wrapResponse(async () => {
      const result = await this.request<PaymentDetails>(`/payments/${paymentId}`);
  ...
  getEnvironment(): Environment {
    return this.baseUrl.includes(BASE_URLS.sandbox)
   ...
   */
  async initPaymentOrThrow(request: InitPaymentRequest): Promise<InitPaymentResponse> {
    const mergedRequest = this.mergeWithDefaults(request);
    this.validateInitPaymentRequest(mergedRequest);
    return this.request<InitPaymentResponse>("/payments/init-payment", {
      method: "POST",
