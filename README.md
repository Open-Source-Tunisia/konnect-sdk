...
import { env } from '@/env';
import { KonnectSDK } from '@konnect/payment-sdk';
...
const konnect = new KonnectSDK({
  ...
  defaults: {
    receiverWalletId: env.KONNECT_WALLET_ID,
    webhook: `https://yourapp.com/webhooks/konnect`,
    theme: 'dark',
  },
});
...
app.post('/api/payments', async (c) => {
  const { amount, orderId, email } = await c.req.json();
  ...
  return c.json(
    { payUrl: data.payUrl, paymentRef: data.paymentRef },
    201
  );
});

// Check payment status
app.get('/api/payments/:paymentRef', async (c) => {
...
// Webhook handler
app.post('/webhooks/konnect', async (c) => {
